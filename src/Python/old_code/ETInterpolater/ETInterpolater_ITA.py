...
class ReadQuantities:
    ...
    def read(self, name):
        for i, q in enumerate(self.quantity_instances):
            ...
            if self.geometries == None or len(self.geometries) == 1:
                print "[~] Starting to Read %s on Grid with None geometry" 
            else:
    ...
    def test_plot(self, quantity="alp"):
        ...
        plt.pcolormesh(x,y,np.log(np.abs(q_diff)))
        plt.colorbar()
        plt.title(r"$|g_{xx}^{simulated}-g_{xx}^{analytical}|$")
        plt.show()

        plt.contour(x,y,np.log(np.abs(q_diff)))
        plt.title(r"$|g_{xx}^{simulated}-g_{xx}^{analytical}|$")
        plt.show()
        ...
        if quantity == "gxx":
            ...
            plt.semilogy(x, np.abs(one_dim_plot[:,2]))
            plt.show()
    ...
    def read(self, name):
        ...
        if self._check_pickle(name):
            ...
            if self.geo is None:
                ...
                self.loaded_spline = grid.spline(order=4, mode="nearest")
    ...
    def __call__(self, coords, output=None):
        ...
        if self.name in ["alp", "gxx", "gyy", "gzz"]:
            if sqrt(coords[0]**2 + coords[1]**2 + coords[2]**2) > 280:
                return 1
            elif sqrt(coords[0]**2 + coords[1]**2 + coords[2]**2) < 0.5:
                return q([0.5,0,0])
...
if __name__=="__main__":
    ...
    inter.make_schwarzschild_isotropic(do_gyoto_converstion=True)
    exit()
    """
    """
    ...
    #g1 = inter.make_positive_geometry([-200,-200, -200], 100)
    #g1 = inter.make_positive_geometry([-200,-200, -200], 100)
    #g2 = inter.make_positive_geometry([-400,-400, -400], 200)
    ...
    #et_q = ReadQuantities([g0,g15,g1,g2], it, folder, pickle_folder=pickle_folder, pickle=False, linear=False, limits=limits)
    et_q = ReadQuantities([g], it, folder, pickle_folder=pickle_folder, pickle=False, linear=False, limits=limits)
    et_q.test_plot("gxx")
    #exit()
