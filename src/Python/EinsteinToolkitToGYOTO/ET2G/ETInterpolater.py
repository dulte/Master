...
from ReadQuantities import ReadQuantities
...
class ETInterpolater:
    """
    This class will take an objects with Einstein Toolkit data and
    use C code build around LORENE to find the collocation
    ...
    def read_ET_quantity(self,quantity, geometry, iteration, dimentions=3, order=4):
        """
        This is an old method which isn't in use anymore. Use the ReadQuantities class instead!
    ...
    def make_semipositive_geometry(self, corner, n_pts):
        ...
        Einstein Toolkit simulation is done with symmetry in xz!
...
quantities[quantity]
    ...
    def _make_analytical_metric(self, analytical_function, c_path="../../C/", result_path="./", do_gyoto_converstion=True):
        ...
        analytic_function : function
            The function which generates the analytical metric
        c_path : str, optional
            The path to the C file (default is '../../C/')
        result_path : str, optional 
            Path for results (default is './')
        do_gyoto_conversion : bool, optional
            Whether or not to do the last conversion (default is True)
    ...
    def get_minkowski_component(self,quantity):
        """
        Returns a function for the metric components for a Minkowski metric
    ...
    def _non_zero_schwarzschild_alp(self, x):
        """
        Returns the lapse for a Schwarzschild metric
    ...
    def _non_zero_schwarzschild_gamma(self, x):
        """
        Returns the spatial metricfor a Schwarzschild metric
    ...
    def get_schwarzschild_isotropic_compinents(self, quantity):
        """
        Returns a function for the metric components for a Schwarzschild metric
        ...
        -------
        function
            Lambda function for components
        """
        if quantity == "alp":
